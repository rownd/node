...
import { CLAIM_USER_ID, fetchAppConfig, fetchRowndJwks, fetchRowndWellKnownConfig } from "./core";
import { createSmartLink } from "./smart_links";
import { CreateSmartLinkOpts, FetchUserInfoOpts, IRowndClient, IRowndExpressClient, RowndToken, RowndUser, TApp, TConfig } from "../types";
...
export class RowndInstance implements IRowndClient {
    ...
    async fetchUserInfo(opts: FetchUserInfoOpts) {
        ...
        let resp: Record<string, any> = await got
            .get(`${this.config.api_url}/applications/${appId}/users/${userId}/data`, {
                headers,
            })
            .json();
    ...
    async createOrUpdateUser(user: RowndUser) {
        ...
        let resp: RowndUser = await got
            .put(
                `${this.config.api_url}/applications/${this.config._app!.id}/users/${user.id || user.data.user_id}/data`,
                {
                    headers: {
    ...
    async deleteUser(userId: String) {
        await got.delete(
            `${this.config.api_url}/applications/${this.config._app!.id}/users/${userId}/data`,
            {
              headers: {
                'x-rownd-app-key': this.config.app_key,
                'x-rownd-app-secret': this.config.app_secret,
