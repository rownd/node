...
export default function expressLib(config: TConfig) {
  ...
  function authenticate(opts: AuthenticateOpts) {
    ...
    return (req: RowndRequest, _: express.Response, next: express.NextFunction) => {
      ...
      (async (req: RowndRequest, _: express.Response, next: express.NextFunction) => {
        try {
          ...
          let wrappingError = new WrappedError(`The provided token was invalid. Reason: ${(err as Error).message}`);
