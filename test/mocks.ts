...
// Init JWK
let keyPair: Promise<jose.GenerateKeyPairResult> = jose.generateKeyPair('EdDSA', { crv: 'Ed25519' });
...
const handlers = [
    rest.get('https://mock-api.local/hub/auth/.well-known/oauth-authorization-server', (_, res, ctx) => {
        return res(
            ctx.json({
    ...
    rest.put('https://mock-api.local/applications/290167281732813315/users/rownd-test-user-1/data', (_, res, ctx) => {
        return res(
            ctx.json({
                "data":{
